...
This chapter discussed the results presented in @sec-results, where it has been shown that the district heating system based on solar thermal energy and seasonal storage compares favorably to other means of heating, but in the given modelling, with limitations as presented in @sec-model-limitation, exhibits higher environmental impact compared to the district heating system based on wood chips as the energy sources. The electricity and the supplemental biomass required were found to be the two top single contributors to the overall results. To understand the saving potential from using less or lower-emitting resources, sensitivity analyses for source of electricity (see @sec-sensitive-elec) and loss coefficients for the heat loss in the seasonal storage unit and the transport pipes (see third research question and @sec-sensitive-loss) are performed.

## Modeling limitations {#sec-model-limitation}

The detailed data of the SimaPro process named *district heat, at consumer, solar thermal, 1MW/MJ/RER U* that was defined for this analysis can be found in @fig-sima-pro-data. Data availability was generally satisfactory with one notable exception for the modelling of the heat pumps used in the system (see @sec-hp for specifications). There are two heat pumps with 630 kW max electrical input power each used in the system, but in the UVEK data the largest sized heat pump has a maximum of 50 kW power. The assumption is that the two large heat pump have the same impact as 25 units of the 50 kW heat pump, likely leading an overestimation of the resource use for minerals and metals.
