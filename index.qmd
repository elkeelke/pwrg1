...
The results for comparing the solar thermal based district network heat it to other forms of district heating that are available in small communities such as wood chips (biomass), geothermal heat pump or diesel cogeneration, and also to the usually available consumer side heating installations such as a oil or gas boiler are shown in @fig-es-single-score and @fig-ef-midpoints.

![Comparison to other energy sources for district heating (wood chips, heatpump) and consumer installations (gas, oil), based on the Ecological Scarcity method at the single score level](img/pwrg_solar_es_single_score.png){#fig-es-single-score width="100%"}

![Damage assessment using the Ecological Footprint Method](img/pwrg_solar_ef_midpoints.png){#fig-ef-midpoints width="100%"}
...
![Relative influence of the system components using the Ecological Scarcity 2021 v1.04 method with UVEK weighting set](img/pwrg_solar_network_pie.png){#fig-network-pie width="100%"}

The relative influence of the district heating system components can be seen in @fig-network-pie.

# Discussion

As shown in @fig-es-single-score, the STC based district heating with seasonal heat storage compares positively to other heating technologies in terms of environmental impact. Based on the analysis using the Ecological Scarcity method, it shows a lower environmental impact compared to other district heating setups and to fossil energy based consumer installations.

TBD describe @fig-ef-midpoints
