...
This is an additionally performed analysis for the first research question
...
Given these reference numbers from literature, the 50% loss assumption for Bracht is on the conservative side. We assume that it can be lowered to 30% based on two different scenarios (1) without any changes in the setup but an understanding that the original assumptions were not accurate, and (2) by using twice the cover for the top of seasonal storage unit and therefore better insulation (with the sides and the bottom unchanged). 

For both scenarios the result would be a higher amount energy supplied from the seasonal storage unit (3.160 MWh), and consequently an increase in electrical energy requirement for the heat pump (293 MWh). We perform a rough estimate of the heightened energy requirements based on using the same factor between energy from the seasonal storage unit and energy from electricity as in the original assumptions (see @sec-loss-coefficients): 209 MWh divided by 2.258 MWh, which returns a 9 % electricity requirement factor. With an assumed 20% heat loss in the pipes as in the default assumptions, a remaining amount of 2.764 MWh of energy is available for consumers. Therefore, no supplemental energy from biomass would be required. 

For scenario (1 - no changes), a 15 % reduction from 170 Pt per kWh of heat to 146 Pt per kWh of heat can be achieved. For scenario (2 - double cover), a 9 % reduction to to 155[^note] Pt per kWh of heat can be achieved.
