...
*   26 % from biomass sources (904 MWh), and
*.  6 % from the electricity grid (209 MWh).

While detailed methods for estimating losses are not provided in the documentation, the modelling assumes a 20% loss coefficient for the pipe network. This estimate was chosen based on an initial literature study and previous experience. Given this assumption, the implied thermal loss within the seasonal storage unit would be approximately 50% to align with the reported figures from @bracht2025faq. These assumptions were incorporated into the system model as the default values. A more detailed discussion of thermal loss coefficients and their influence on system performance is presented in the sensitivity analysis in @sec-sensitive-loss.
