...
long_ch <- long_ch |>
  mutate(
    ...
    label = ifelse(category == baseline_category, "Baseline", sprintf("%+.1f%%", pct_change))
  )
...
data <- data.frame(
  category = c("A", "B", "C", "D"),
  value = c(100, 120, 80, 110)
)

# Define baseline (e.g., category A)
baseline_category <- "A"
baseline_value <- data %>% filter(category == baseline_category) %>% pull(value)
...
ggplot(data, aes(x = category, y = value, fill = category)) +
  ...
  annotate("text", x = Inf, y = baseline_value, label = paste("Baseline =", baseline_value),
           hjust = 1.1, vjust = -0.5, color = "black", size = 3.5) +
  theme_minimal() +
  labs(title = "Bar Chart with Percentage Change from Baseline",
       y = "Value",
       x = "Category")
