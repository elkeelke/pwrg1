...
long_es <- long_es |>
  mutate(
    ...
    Type = ifelse(Type == "DH: Solar thermal", "Bracht plant", Type),
  )
...
## ES single score bar chart for presentation
```{r}
es <- read_delim("data/PWRG 1 Elke Michlmayr_ Result data for charts - Fig 4 final.csv")[1:7, ]
...
ggplot(long_es, aes(x = Type, y = Value, fill = Type)) +
  geom_bar(stat = "identity", width = 0.6) +  # Use 'identity' to plot actual values
  labs(x = "",
       y = "Pt per MJ") +
  scale_fill_brewer(palette = "Blues") +
...
ggsave("img/pwrg_solar_es_single_score_total.png")
```
